"use client";

import { motion } from "framer-motion";
import Image from "next/image";

export default function About() {
  return (
    <section id="about" className="py-24 bg-purple-50 overflow-hidden relative">
      {/* Decorative Blob */}
      <div className="absolute top-0 right-0 -mt-20 -mr-20 w-96 h-96 bg-purple-200 rounded-full mix-blend-multiply filter blur-3xl opacity-50 pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-16">
          <motion.div
            initial={{ opacity: 0, x: -50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="w-full lg:w-1/2"
          >
            <div className="relative h-[500px] w-full rounded-3xl overflow-hidden shadow-2xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-950 flex flex-col justify-between p-6 border border-purple-500/20 group">
              {/* Fabric texture background patterns */}
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(99,102,241,0.08)_0%,transparent_70%)] pointer-events-none" />
              
              {/* Fine weaving vertical & horizontal basting thread grid */}
              <div 
                className="absolute inset-0 opacity-10 pointer-events-none"
                style={{
                  backgroundImage: `
                    linear-gradient(rgba(139, 92, 246, 0.2) 1px, transparent 1px),
                    linear-gradient(90deg, rgba(139, 92, 246, 0.2) 1px, transparent 1px)
                  `,
                  backgroundSize: '16px 16px'
                }}
              />

              {/* Stitched outer border inside the box */}
              <div className="absolute inset-4 rounded-2xl border border-dashed border-purple-500/30 pointer-events-none" />

              {/* Main SVG Tailoring Map Canvas */}
              <svg viewBox="155 540 110 135" className="w-full h-[320px] object-contain select-none z-10">
                <defs>
                  {/* Glowing radial drop shadow */}
                  <filter id="glow" x="-20%" y="-20%" width="140%" height="140%">
                    <feGaussianBlur stdDeviation="3" result="blur" />
                    <feComposite in="SourceGraphic" in2="blur" operator="over" />
                  </filter>

                  {/* Denim fabric cross-stitch weave pattern */}
                  <pattern id="denimPattern" width="4" height="4" patternUnits="userSpaceOnUse">
                    <path d="M 0,2 L 4,2 M 2,0 L 2,4" stroke="#4f46e5" strokeWidth="0.5" strokeOpacity="0.25" />
                  </pattern>

                  {/* Metallic needle gradient */}
                  <linearGradient id="needleGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stopColor="#f8fafc" />
                    <stop offset="50%" stopColor="#cbd5e1" />
                    <stop offset="100%" stopColor="#64748b" />
                  </linearGradient>

                  {/* Golden stitching gradient */}
                  <linearGradient id="goldGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="#f59e0b" />
                    <stop offset="100%" stopColor="#fbbf24" />
                  </linearGradient>
                </defs>

                {/* Soft backdrop glow behind map */}
                <circle cx="211" cy="609" r="45" fill="#6366f1" opacity="0.08" filter="url(#glow)" />

                {/* Chalk sewing layout guidelines */}
                <path d="M 158,565 Q 200,550 255,575" fill="none" stroke="#8b5cf6" strokeWidth="0.4" strokeOpacity="0.2" strokeDasharray="3 3" />
                <path d="M 175,655 Q 215,615 245,662" fill="none" stroke="#8b5cf6" strokeWidth="0.4" strokeOpacity="0.2" strokeDasharray="3 3" />

                {/* Tape measure ticks on the left border */}
                {Array.from({ length: 26 }).map((_, idx) => {
                  const y = 542 + idx * 5;
                  const isMajor = idx % 2 === 0;
                  return (
                    <line
                      key={`tick-l-${idx}`}
                      x1="156"
                      y1={y}
                      x2={isMajor ? "160" : "158"}
                      y2={y}
                      stroke="#8b5cf6"
                      strokeWidth="0.4"
                      strokeOpacity="0.25"
                    />
                  );
                })}

                {/* Tape measure ticks on the right border */}
                {Array.from({ length: 26 }).map((_, idx) => {
                  const y = 542 + idx * 5;
                  const isMajor = idx % 2 === 0;
                  return (
                    <line
                      key={`tick-r-${idx}`}
                      x1="264"
                      y1={y}
                      x2={isMajor ? "260" : "262"}
                      y2={y}
                      stroke="#8b5cf6"
                      strokeWidth="0.4"
                      strokeOpacity="0.25"
                    />
                  );
                })}

                {/* Scissors silhouette in background */}
                <g transform="translate(162, 548) rotate(25)" opacity="0.2" stroke="#cbd5e1" strokeWidth="0.6" fill="none" strokeLinecap="round">
                  <circle cx="2" cy="2" r="1.5" />
                  <circle cx="2" cy="-2" r="1.5" />
                  <circle cx="5" cy="0" r="0.4" fill="#cbd5e1" />
                  <line x1="5" y1="0" x2="14" y2="0.8" />
                  <line x1="5" y1="0" x2="14" y2="-0.8" />
                </g>

                {/* Main Tamil Nadu Map Fabric Shape */}
                <motion.path
                  d="m 232.906,641.22647 0.477,0.35867 -0.431,0.6186 0.239,0.43726 2.384,2.20037 -2.954,-1.881 -1.762,-0.15314 -0.215,-0.49568 0.337,-0.31334 0.988,-0.0232 0.937,-0.74857 z m -22.853,-73.11506 0.137,0.80398 -0.782,0.72742 1.561,-0.22669 0.545,0.83622 1.63,0.88257 1.911,-0.11989 -0.041,-0.96317 0.647,-0.0222 0.102,-0.86241 0.358,0.0363 0.114,-0.0373 0.176,0.51281 0.487,-0.12694 -0.044,-1.25333 0.762,-0.57729 0.916,-3.18974 0.439,0.004 -0.691,-1.28657 0.784,-0.83925 0.893,0.29117 0.182,-1.11328 0.912,0.23071 2.722,-0.87753 0.123,1.42964 0.895,0.0232 -0.653,-0.35463 0.438,-0.70223 1.365,-0.1884 1.873,1.34098 1.751,0.14709 0.758,-1.66338 0.615,-0.41508 0.001,-0.68107 1.027,-0.12191 0.849,0.95309 0.976,-0.70625 -0.142,-0.71633 1.591,-0.52491 -0.045,-0.45337 0.5,-0.17732 -0.197,-0.28109 -0.529,0.38083 0.297,-1.49009 -1.056,-0.46748 1.136,-0.44229 2.238,0.27001 0.271,0.94604 0.66,0.46849 1.848,-0.78686 0.1,1.12941 0.476,-0.0957 -0.153,0.57025 0.714,-0.31132 0.451,0.57931 0.848,-0.61457 -1.332,-1.04981 0.687,-0.6579 0.538,0.44632 0.429,-0.47957 1.065,0.0191 1.556,-0.70021 0.587,-0.82111 -0.441,-0.65085 0.644,-0.29217 -0.029,-0.71935 0.702,-0.17632 0.22,-0.60953 0.631,0.0705 -0.069,-0.37378 -0.891,-0.17833 0.083,-0.42113 1.577,-0.17228 1.73,1.26642 1.638,0.0433 0.956,1.89007 0.756,0.42516 -0.001,-1.1425 0,0 0.687,3.21392 -0.959,2.88144 -0.924,7.65899 -2.047,6.72907 -5.699,8.68463 -0.988,2.66382 0,0 -0.832,-0.49267 -1.277,1.20698 -0.444,-1.28959 0.772,-0.25792 -0.375,-0.31736 -0.604,0.36269 -0.561,-0.89768 -0.725,0.70626 0,0 0,0 0,0 1.144,0.30527 -0.437,0.75965 0.583,0.16826 0.395,0.79592 -0.199,0.35464 -0.681,-0.63674 -0.499,0.83622 0.937,0.58637 0.963,-0.47655 0.049,0.77275 0.62,-0.33852 0.39,0.68308 0,0 -0.907,5.23496 1.698,4.46623 0.343,8.09121 0,0 -1.112,-0.21762 -0.377,0.403 -1.078,-0.42113 0.285,0.93193 -0.877,-0.18135 0.486,1.16064 1.719,0.64076 0.091,1.13848 0.793,0.007 0,0 0.587,11.02403 -0.913,0.44934 -1.942,0.006 -7.478,-0.88761 -1.901,1.1949 -1.378,2.47844 -0.012,1.11227 0.814,1.15258 -2.35,2.24571 -0.697,1.65431 -2.675,3.10712 -1.244,2.14295 -0.762,2.70815 0.861,1.57069 2.599,2.24672 2.657,0.39997 -2.023,0.46043 -2.902,-0.26598 -1.715,0.3365 -3.047,1.01455 -0.192,0.39494 -1.268,-0.0473 -0.326,0.39595 0.314,0.34254 -1.657,0.56017 -3.506,0.41207 -4.221,3.23608 -0.844,1.63718 -0.238,2.34445 0.622,-0.10579 0.901,0.58939 -1.273,0.0997 -1.349,1.96059 -0.175,1.49009 0.884,-0.15919 -0.174,1.94245 -1.367,1.63618 -0.325,0.83521 0.366,0.20553 -5.506,2.5933 -1.198,1.50016 -3.582,0.88156 -0.518,1.25132 -4.919,-0.94 -4.595,-3.7489 0,0 0.533,-0.52692 0.79,0.22165 -0.52,-1.39941 1.563,-1.25333 -0.481,-1.17877 1.278,-0.16926 0.369,-0.76469 -2.299,-4.24055 0.444,0.0544 0.798,-0.86342 0.593,-1.77118 -0.801,-1.23318 -0.508,0.0353 -0.005,-0.66293 -1.005,-1.37523 1.045,-0.5239 0.288,-1.1022 1.17,-1.15762 -0.317,-0.76771 0.632,-0.62968 0,-1.74197 0.362,-0.77577 0.569,-0.0101 0.48,-2.19131 0.928,-0.41811 0.37,-0.97324 -0.957,-0.6317 -0.202,-1.46087 -0.337,0.67703 -0.429,-0.63371 -1.032,0.59644 -1.237,-0.8332 -0.922,-0.0393 0.796,-2.32832 0.383,-0.13098 0.033,-1.23821 0.506,-0.18336 -0.524,-0.55312 0.309,-0.73346 -0.514,-0.36068 0.745,-1.63416 0.509,-0.22467 -0.229,-1.42259 -1.262,-1.53845 1.507,-0.56621 0.433,-1.87596 -0.71,-0.28411 0.084,-0.70525 -0.755,-1.10926 0.16,-0.73446 -1.239,-0.12795 -2.326,1.29463 -0.279,0.79492 -1.418,0.84125 -1.367,-0.34355 -0.904,-0.9007 0.042,-0.48965 -0.539,0.26497 -0.395,-0.37478 0.412,-1.37222 -0.67,-1.17675 0.469,-3.62598 -0.518,-0.88056 1.416,-0.0655 -0.358,-0.97727 0.879,-2.03616 -0.193,-0.62061 -1.002,-0.16624 -0.479,-1.15459 -3.525,-1.33091 1.201,-2.28802 1.799,-0.23475 -1.159,-0.10881 0.034,-1.57572 -0.996,-0.49569 0.72,-1.37423 -0.599,-0.48057 -1.547,0.97626 -2.834,-0.36572 -0.73,0.48259 -0.133,-1.07399 1.963,-1.32285 -0.042,-1.26642 -0.601,0.10579 -1.279,-0.74253 -0.795,-1.1153 -1.606,-0.53699 -0.268,0.30023 -0.897,-0.63875 -0.794,0.31837 -0.3,-2.24067 0.946,-0.60853 0.564,0.61054 1.093,-0.39393 0.626,-0.84932 0.937,-0.0191 -0.028,-0.77175 0,0 0.584,0.0897 1.106,-0.91783 0.758,0.58939 0.412,1.29161 3.897,-0.009 1.471,0.28512 0.435,0.48562 0.285,-0.58435 -0.469,-0.90473 1.49,-2.97817 1.254,0.38789 0.276,-0.76872 0.663,-0.0413 0.57,0.40804 -0.195,0.41106 0.716,-0.26598 0.316,1.11429 0.789,0.26195 -0.136,-1.07601 2.601,-0.44128 0.308,-0.39998 0.902,0.0877 0.864,0.84327 0.699,-0.42416 1.251,0.60148 0.775,-1.89309 -0.073,-1.05888 0.505,0.26901 0.157,-1.20799 3.686,-0.12292 0.607,-1.53744 1.654,-2.04925 -0.881,-1.32284 -5.457,-0.74958 0.095,-1.27851 1.518,-0.46849 1.279,-1.50318 0.34,-1.17273 0.107,-1.47699 -1.043,0.19042 0.649,-0.67402 -0.583,-0.15515 -0.177,-1.09314 0.582,-1.35407 -0.157,-0.78484 1.339,-0.35666 0.504,0.54002 0.533,-0.52994 0.578,0.20956 1.425,-2.63965 -0.306,-1.00951 2.781,-0.95813 0.35,0.48058 -0.528,0.73245 0.326,0.17833 0.435,-0.7657 0.75,1.15459 0.42,0.004 0.281,-1.08407 0.666,0.0756 1.548,1.55456 2.088,0.37177 z"
                  fill="url(#denimPattern)"
                  fillOpacity="0.7"
                  stroke="#4f46e5"
                  strokeWidth="1.2"
                  filter="url(#glow)"
                  initial={{ pathLength: 0, opacity: 0.3 }}
                  whileInView={{ pathLength: 1, opacity: 1 }}
                  viewport={{ once: true }}
                  transition={{ duration: 1.8, ease: "easeOut" }}
                />

                {/* Stitched seam line traced in contrast gold thread */}
                <motion.path
                  d="m 232.906,641.22647 0.477,0.35867 -0.431,0.6186 0.239,0.43726 2.384,2.20037 -2.954,-1.881 -1.762,-0.15314 -0.215,-0.49568 0.337,-0.31334 0.988,-0.0232 0.937,-0.74857 z m -22.853,-73.11506 0.137,0.80398 -0.782,0.72742 1.561,-0.22669 0.545,0.83622 1.63,0.88257 1.911,-0.11989 -0.041,-0.96317 0.647,-0.0222 0.102,-0.86241 0.358,0.0363 0.114,-0.0373 0.176,0.51281 0.487,-0.12694 -0.044,-1.25333 0.762,-0.57729 0.916,-3.18974 0.439,0.004 -0.691,-1.28657 0.784,-0.83925 0.893,0.29117 0.182,-1.11328 0.912,0.23071 2.722,-0.87753 0.123,1.42964 0.895,0.0232 -0.653,-0.35463 0.438,-0.70223 1.365,-0.1884 1.873,1.34098 1.751,0.14709 0.758,-1.66338 0.615,-0.41508 0.001,-0.68107 1.027,-0.12191 0.849,0.95309 0.976,-0.70625 -0.142,-0.71633 1.591,-0.52491 -0.045,-0.45337 0.5,-0.17732 -0.197,-0.28109 -0.529,0.38083 0.297,-1.49009 -1.056,-0.46748 1.136,-0.44229 2.238,0.27001 0.271,0.94604 0.66,0.46849 1.848,-0.78686 0.1,1.12941 0.476,-0.0957 -0.153,0.57025 0.714,-0.31132 0.451,0.57931 0.848,-0.61457 -1.332,-1.04981 0.687,-0.6579 0.538,0.44632 0.429,-0.47957 1.065,0.0191 1.556,-0.70021 0.587,-0.82111 -0.441,-0.65085 0.644,-0.29217 -0.029,-0.71935 0.702,-0.17632 0.22,-0.60953 0.631,0.0705 -0.069,-0.37378 -0.891,-0.17833 0.083,-0.42113 1.577,-0.17228 1.73,1.26642 1.638,0.0433 0.956,1.89007 0.756,0.42516 -0.001,-1.1425 0,0 0.687,3.21392 -0.959,2.88144 -0.924,7.65899 -2.047,6.72907 -5.699,8.68463 -0.988,2.66382 0,0 -0.832,-0.49267 -1.277,1.20698 -0.444,-1.28959 0.772,-0.25792 -0.375,-0.31736 -0.604,0.36269 -0.561,-0.89768 -0.725,0.70626 0,0 0,0 0,0 1.144,0.30527 -0.437,0.75965 0.583,0.16826 0.395,0.79592 -0.199,0.35464 -0.681,-0.63674 -0.499,0.83622 0.937,0.58637 0.963,-0.47655 0.049,0.77275 0.62,-0.33852 0.39,0.68308 0,0 -0.907,5.23496 1.698,4.46623 0.343,8.09121 0,0 -1.112,-0.21762 -0.377,0.403 -1.078,-0.42113 0.285,0.93193 -0.877,-0.18135 0.486,1.16064 1.719,0.64076 0.091,1.13848 0.793,0.007 0,0 0.587,11.02403 -0.913,0.44934 -1.942,0.006 -7.478,-0.88761 -1.901,1.1949 -1.378,2.47844 -0.012,1.11227 0.814,1.15258 -2.35,2.24571 -0.697,1.65431 -2.675,3.10712 -1.244,2.14295 -0.762,2.70815 0.861,1.57069 2.599,2.24672 2.657,0.39997 -2.023,0.46043 -2.902,-0.26598 -1.715,0.3365 -3.047,1.01455 -0.192,0.39494 -1.268,-0.0473 -0.326,0.39595 0.314,0.34254 -1.657,0.56017 -3.506,0.41207 -4.221,3.23608 -0.844,1.63718 -0.238,2.34445 0.622,-0.10579 0.901,0.58939 -1.273,0.0997 -1.349,1.96059 -0.175,1.49009 0.884,-0.15919 -0.174,1.94245 -1.367,1.63618 -0.325,0.83521 0.366,0.20553 -5.506,2.5933 -1.198,1.50016 -3.582,0.88156 -0.518,1.25132 -4.919,-0.94 -4.595,-3.7489 0,0 0.533,-0.52692 0.79,0.22165 -0.52,-1.39941 1.563,-1.25333 -0.481,-1.17877 1.278,-0.16926 0.369,-0.76469 -2.299,-4.24055 0.444,0.0544 0.798,-0.86342 0.593,-1.77118 -0.801,-1.23318 -0.508,0.0353 -0.005,-0.66293 -1.005,-1.37523 1.045,-0.5239 0.288,-1.1022 1.17,-1.15762 -0.317,-0.76771 0.632,-0.62968 0,-1.74197 0.362,-0.77577 0.569,-0.0101 0.48,-2.19131 0.928,-0.41811 0.37,-0.97324 -0.957,-0.6317 -0.202,-1.46087 -0.337,0.67703 -0.429,-0.63371 -1.032,0.59644 -1.237,-0.8332 -0.922,-0.0393 0.796,-2.32832 0.383,-0.13098 0.033,-1.23821 0.506,-0.18336 -0.524,-0.55312 0.309,-0.73346 -0.514,-0.36068 0.745,-1.63416 0.509,-0.22467 -0.229,-1.42259 -1.262,-1.53845 1.507,-0.56621 0.433,-1.87596 -0.71,-0.28411 0.084,-0.70525 -0.755,-1.10926 0.16,-0.73446 -1.239,-0.12795 -2.326,1.29463 -0.279,0.79492 -1.418,0.84125 -1.367,-0.34355 -0.904,-0.9007 0.042,-0.48965 -0.539,0.26497 -0.395,-0.37478 0.412,-1.37222 -0.67,-1.17675 0.469,-3.62598 -0.518,-0.88056 1.416,-0.0655 -0.358,-0.97727 0.879,-2.03616 -0.193,-0.62061 -1.002,-0.16624 -0.479,-1.15459 -3.525,-1.33091 1.201,-2.28802 1.799,-0.23475 -1.159,-0.10881 0.034,-1.57572 -0.996,-0.49569 0.72,-1.37423 -0.599,-0.48057 -1.547,0.97626 -2.834,-0.36572 -0.73,0.48259 -0.133,-1.07399 1.963,-1.32285 -0.042,-1.26642 -0.601,0.10579 -1.279,-0.74253 -0.795,-1.1153 -1.606,-0.53699 -0.268,0.30023 -0.897,-0.63875 -0.794,0.31837 -0.3,-2.24067 0.946,-0.60853 0.564,0.61054 1.093,-0.39393 0.626,-0.84932 0.937,-0.0191 -0.028,-0.77175 0,0 0.584,0.0897 1.106,-0.91783 0.758,0.58939 0.412,1.29161 3.897,-0.009 1.471,0.28512 0.435,0.48562 0.285,-0.58435 -0.469,-0.90473 1.49,-2.97817 1.254,0.38789 0.276,-0.76872 0.663,-0.0413 0.57,0.40804 -0.195,0.41106 0.716,-0.26598 0.316,1.11429 0.789,0.26195 -0.136,-1.07601 2.601,-0.44128 0.308,-0.39998 0.902,0.0877 0.864,0.84327 0.699,-0.42416 1.251,0.60148 0.775,-1.89309 -0.073,-1.05888 0.505,0.26901 0.157,-1.20799 3.686,-0.12292 0.607,-1.53744 1.654,-2.04925 -0.881,-1.32284 -5.457,-0.74958 0.095,-1.27851 1.518,-0.46849 1.279,-1.50318 0.34,-1.17273 0.107,-1.47699 -1.043,0.19042 0.649,-0.67402 -0.583,-0.15515 -0.177,-1.09314 0.582,-1.35407 -0.157,-0.78484 1.339,-0.35666 0.504,0.54002 0.533,-0.52994 0.578,0.20956 1.425,-2.63965 -0.306,-1.00951 2.781,-0.95813 0.35,0.48058 -0.528,0.73245 0.326,0.17833 0.435,-0.7657 0.75,1.15459 0.42,0.004 0.281,-1.08407 0.666,0.0756 1.548,1.55456 2.088,0.37177 z"
                  fill="none"
                  stroke="url(#goldGrad)"
                  strokeWidth="1.2"
                  strokeDasharray="3 3"
                  initial={{ pathLength: 0 }}
                  whileInView={{ pathLength: 1 }}
                  viewport={{ once: true }}
                  transition={{ duration: 2.2, ease: "easeOut", delay: 0.3 }}
                />

                {/* Glowing red thread leading to the needle and the workshop pin */}
                <motion.path
                  d="M 172,550 C 180,538 190,555 198,563 C 200,565 210,578 224.5,591.1"
                  fill="none"
                  stroke="#ef4444"
                  strokeWidth="0.8"
                  strokeLinecap="round"
                  initial={{ pathLength: 0 }}
                  whileInView={{ pathLength: 1 }}
                  viewport={{ once: true }}
                  transition={{ duration: 1.5, ease: "easeOut", delay: 1 }}
                />

                {/* Needle that represents active tailoring */}
                <motion.g
                  initial={{ x: -5, y: -5, rotate: -5 }}
                  whileInView={{ x: 0, y: 0, rotate: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 1.2, ease: "easeOut", delay: 0.8 }}
                >
                  {/* Needle shadow */}
                  <line x1="202" y1="567" x2="226.5" y2="593.1" stroke="#020617" strokeWidth="1" strokeOpacity="0.4" strokeLinecap="round" />
                  {/* Needle shaft */}
                  <line x1="200" y1="565" x2="224.5" y2="591.1" stroke="url(#needleGrad)" strokeWidth="1" strokeLinecap="round" />
                  {/* Needle eye */}
                  <ellipse cx="201.5" cy="566.5" rx="0.3" ry="1" transform="rotate(-45 201.5 566.5)" fill="#0f172a" />
                </motion.g>

                {/* Stitched wooden button representing Chinnasalem workshop location */}
                <g className="cursor-pointer">
                  {/* Pulsing thread stitch aura */}
                  <circle cx="224.5" cy="591.1" r="5" fill="#f59e0b" opacity="0.3">
                    <animate attributeName="r" values="3.5;7.5;3.5" dur="3s" repeatCount="indefinite" />
                    <animate attributeName="opacity" values="0.4;0;0.4" dur="3s" repeatCount="indefinite" />
                  </circle>
                  
                  {/* Wood-finish circle backing */}
                  <circle cx="224.5" cy="591.1" r="3" fill="#b45309" stroke="#fef08a" strokeWidth="0.4" />
                  {/* Button stitch holes */}
                  <circle cx="223.7" cy="590.3" r="0.3" fill="#451a03" />
                  <circle cx="225.3" cy="590.3" r="0.3" fill="#451a03" />
                  <circle cx="223.7" cy="591.9" r="0.3" fill="#451a03" />
                  <circle cx="225.3" cy="591.9" r="0.3" fill="#451a03" />
                  {/* Yellow thread criss-cross stitches */}
                  <line x1="223.7" y1="590.3" x2="225.3" y2="591.9" stroke="#fef08a" strokeWidth="0.3" />
                  <line x1="223.7" y1="591.9" x2="225.3" y2="590.3" stroke="#fef08a" strokeWidth="0.3" />
                </g>

                {/* Dashed connector line to workshop label tag */}
                <line x1="224.5" y1="591.1" x2="231" y2="587" stroke="#fbbf24" strokeWidth="0.4" strokeDasharray="1 1" strokeOpacity="0.8" />

                {/* Workshop Label tag */}
                <g transform="translate(231, 580)">
                  <rect x="0" y="0" width="22" height="9" rx="1.5" fill="#ffffff" fillOpacity="0.95" stroke="#cbd5e1" strokeWidth="0.3" filter="drop-shadow(0 1px 2px rgba(0,0,0,0.15))" />
                  <text x="2" y="3.5" fontSize="1.8" fontFamily="Inter, sans-serif" fontWeight="bold" fill="#6b21a8">CHINNASALEM</text>
                  <text x="2" y="6.8" fontSize="1.4" fontFamily="Inter, sans-serif" fontWeight="medium" fill="#b45309">Workshop Hub</text>
                  {/* Tiny stitched border on tag */}
                  <rect x="0.5" y="0.5" width="21" height="8" rx="1" fill="none" stroke="#f59e0b" strokeWidth="0.15" strokeDasharray="0.6 0.6" strokeOpacity="0.6" />
                </g>
              </svg>

              {/* Embossed Leather Brand Label overlay ( Levi's patch style) */}
              <div className="w-full bg-gradient-to-br from-[#854d0e] to-[#713f12] p-4 rounded-xl border border-amber-500/30 shadow-lg text-amber-100 z-20 overflow-hidden relative group-hover:scale-[1.02] transition-transform duration-300">
                <div className="absolute inset-1 border border-dashed border-amber-300/20 rounded-lg pointer-events-none" />
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-[10px] tracking-[0.2em] font-semibold text-amber-400 uppercase">Artisanship Seam</span>
                    <p className="font-serif text-xl font-bold text-white leading-tight">Made in Tamil Nadu</p>
                    <span className="text-xs text-amber-200/80">Chinnasalem, Kallakurichi</span>
                  </div>
                  <div className="flex flex-col items-end text-right">
                    <span className="text-[9px] bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full border border-amber-500/30 uppercase font-mono">100% Handcrafted</span>
                    <span className="text-[10px] text-amber-400 mt-2 font-mono">ESTD 2020</span>
                  </div>
                </div>
              </div>
            </div>
          </motion.div>

          <motion.div
            initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="w-full lg:w-1/2"
          >
            <h2 className="text-primary font-semibold tracking-wider uppercase text-sm mb-2">
              Our Story
            </h2>
            <h3 className="font-serif text-4xl md:text-5xl font-bold text-foreground mb-6">
              The Art of Handmade Excellence
            </h3>
            <p className="text-gray-600 mb-6 leading-relaxed text-lg">
              Raj Prasanth Handmade Fabric Products creates beautiful handmade products from fabric and recycled materials. Every product is stitched with care using tailoring skills and craftsmanship passed down through generations.
            </p>
            <p className="text-gray-600 mb-8 leading-relaxed text-lg">
              We focus on sustainability, quality, handmade excellence, and eco-friendly products. By choosing us, you are not just buying a bag; you are supporting ethical fashion and local artisanship in Kallakurichi.
            </p>

            <div className="grid grid-cols-2 gap-6">
              <div>
                <h4 className="text-4xl font-bold text-primary mb-2">100%</h4>
                <p className="text-gray-600 text-sm font-medium">Handmade with love</p>
              </div>
              <div>
                <h4 className="text-4xl font-bold text-primary mb-2">0%</h4>
                <p className="text-gray-600 text-sm font-medium">Compromise on quality</p>
              </div>
            </div>
          </motion.div>
        </div>
      </div>
    </section>
  );
}
